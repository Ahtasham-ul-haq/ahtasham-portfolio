...
const MagicButton = ({
    ...
    return (
        <button 
            ...
            onClick={handleClick}    
        >
            <span className={`absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)] ${title === "Download CV" && "bg-none"}`} />
            <span 
                className={
                    ...
                    ${otherClasses}
                    ${title === "Download CV" && "bg-white !text-black-100 hover:bg-white/90 transition duration-300"}
                    `
                }>
                {position === "left" && icon}
                {title}
